...
contract Feedbase {
  ...
  mapping(address => mapping(bytes32 => mapping(address => Config)))
    public _config;
  ...
  function read(address src, bytes32 tag)
    public
    view
  ...
  function deposit(address cash, uint256 amt) public payable {
    if (cash == address(0)) {
  ...
  function withdraw(address cash, uint256 amt) public {
    _bals[msg.sender][cash] -= amt;
    if (cash == address(0)) {
  ...
  function balances(address cash, address who) public view returns (uint256) {
